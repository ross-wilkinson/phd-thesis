...
\noindent
The following submitted manuscript has been incorporated as Appendix~\ref{Chap:B}.

\noindent
\textbf{Wilkinson, Ross D.} and Glen A. Lichtwark. 2020. A Method for Tracking Centre of Mass Displacement During Non-seated Cycling Using an Inertial Sensor. SportRxiv. August 27. doi:10.31236/osf.io/htr4k.
...
\begin{table}[h]
	...
	\begin{tabular}{|c|l|l|}
        ...
        & study design and concept & 80 \\
		& data collection & 20 \\
        & data analysis & 60 \\
		& statistical analysis & 20 \\
		& preparation of figures & 20 \\
		& revision of written work & 60 \\
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{@{}lX@{}}
        \toprule
        \textbf{Parameter} & \textbf{Definitions and symbols} \\
        \midrule
        Local- and global-coordinate system & right-handed cartesian \\
        \hdashline[1pt/3pt]
        Global x, y, and z axis definitions & $+x$ to the right, $+y$ straight up, $+z$ toward viewer \\
        \hdashline[1pt/3pt]
        Euler angles & angular position about local $x$, $y$, and $z$ axes defined as bank, heading, and attitude, respectively \\
        \hdashline[1pt/3pt]
        Symbols & $\psi =$ bank, $\theta =$ heading, $\phi =$ attitude \\
        \hdashline[1pt/3pt]
        Angular velocity & about local $x$, $y$, and $z$ axes defined as roll ($\psi_\epsilon$), yaw ($\theta_\epsilon$), and pitch ($\phi_\epsilon$), respectively \\
        \hdashline[1pt/3pt]
        Euler angle rotation order & Heading, attitude, then bank \\
        \hdashline[1pt/3pt]
        Angle units & radians and radians per second \\
        \bottomrule
    \end{tabularx}
    \caption{Notation, reference coordinates, and definitions used to describe the orientation and motion of the IMU and marker cluster body in this study.}
    \label{tab:m4t1}
\end{table}
...
Differences between the IMU and marker cluster's range of vertical displacement were non-normally distributed with no significant linear trend. As such, a non-parametric, constant LoA analysis was performed. Linear regression results and Bland-Altman plots are presented in Figure \ref{fig:m4f4}A and B, respectively. Across all conditions, there was high agreement between the IMU and marker cluster range of vertical displacement (See Table \ref{tab:m4t3}). On average, the IMU marginally overestimated the cluster results by $1.6\pm3.5$ mm (accuracy $\pm$ precision), which equated to an average error of $1.8\pm3.9\%$. Figure \ref{fig:m4f3} illustrates how well the IMU was able to track the marker cluster's vertical displacement during the crank cycle.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}l*{3}{r}@{}}
        \toprule
         & \multicolumn{3}{c}{RMS error} \\
        \cmidrule{2-4}
         & \multicolumn{1}{c}{Yaw ($\theta_\epsilon$)} & \multicolumn{1}{c}{Pitch ($\phi_\epsilon$)} & \multicolumn{1}{c}{Roll ($\psi_\epsilon$)} \\
        \midrule
        $10\%\ P_{max.i}$ & \\
        \hspace{5mm}70 rpm & $0.16 \pm 0.02$ & $0.14 \pm 0.02$ & $0.11 \pm 0.03$ \\
        \hspace{5mm}120 rpm & $0.22 \pm 0.03$ & $0.17 \pm 0.03$ & $0.18 \pm 0.08$ \\
        \hdashline[1pt/3pt]
        $30\%\ P_{max.i}$ & \\
        \hspace{5mm}70 rpm & $0.19 \pm 0.05$ & $0.15 \pm 0.04$ & $0.15 \pm 0.08$ \\
        \hspace{5mm}120 rpm & $0.19 \pm 0.05$ & $0.18 \pm 0.08$ & $0.16 \pm 0.02$ \\
        \hdashline[1pt/3pt]
        $50\%\ P_{max.i}$ & \\
        \hspace{5mm}70 rpm & $0.21 \pm 0.02$ & $0.15 \pm 0.02$ & $0.12 \pm 0.01$ \\
        \hspace{5mm}120 rpm & $0.24 \pm 0.06$ & $0.19 \pm 0.05$ & $0.17 \pm 0.05$ \\
        \bottomrule
    \end{tabular}
    \caption[The sensor performance was excellent across all conditions.]{\textbf{The sensor performance was excellent across all conditions.} Group mean ($\pm$ standard deviation) RMS error in each Euler parameter describing the yaw, pitch, and roll components of the IMU angular velocity (rad$\cdot$s$^{-1}$) compared to the rigid marker cluster body during non-seated cycling at three different power outputs ($10\%$, $30\%$, and $50\%$ $P_{max.i}$) and two different cadences (70 rpm and 120 rpm).}
    \label{tab:m4t2}
\end{table}
...
\begin{table}[htbp]
    \centering
    \ra{1.3}
    \begin{tabularx}{\textwidth}{@{}X*{4}{X}l*{3}{X}@{}}
        \toprule
         & \multicolumn{4}{c}{Agreement analysis} && \multicolumn{3}{c}{Descriptive statistics} \\
         \cmidrule{2-5} \cmidrule{7-9}
         & LloA & UloA & Accuracy (Bias) & Precision (MAD) && Range & Ave. err. & Max. err. \\
        \midrule
        IMU vs Cluster & $-8.3$ & $11.6$ & $1.6$ & $3.5$ && $88.3$ & $1.8\%$ & $\pm3.9\%$ \\
        \hdashline[1pt/3pt]
        IMU vs Model & $0.37x-13.1$ & $0.37x-19.0$ & $0.37x+3.0$ & $5.6$ && - & - & - \\
        \bottomrule
    \end{tabularx}
    \caption[There was good agreement between the IMU and cluster, but not between the IMU and model.]{\textbf{There was good agreement between the IMU and cluster, but not between the IMU and model.} A fixed limits of agreement analysis was used to compare the range of vertical displacement derived from an IMU to a rigid marker cluster. Positive values indicate an overestimation of the marker cluster's vertical displacement by the IMU (IMU$-$Cluster). All measures are in millimetres. A variable limits-of-agreement analysis was used to compare the range of vertical displacement derived from an IMU to the musculoskeletal model. All measures are in millimetres.}
    \label{tab:m4t3}
\end{table}
